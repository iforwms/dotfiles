...
if ! tmux has-session -t domino; then
    ...
    tmux split-window -t domino:1 -v -p 80 -c ~/code/domino/domino-frontend
    ...
    # Split window for main API development
    tmux split-window -t domino:2 -v -p 20 -c ~/code/domino/domino-api

    # Split window for watching CSS changes.
    tmux split-window -t domino:2.2 -h -p 50 -c ~/code/domino/domino-admin

    # Install any missing packages and start a server on port 3001.
    tmux send-keys -t domino:2.2 'yr && PORT=3001 yrs' Enter

    # Watch CSS file for changes.
    tmux send-keys -t domino:2.1 'yr css' Enter
